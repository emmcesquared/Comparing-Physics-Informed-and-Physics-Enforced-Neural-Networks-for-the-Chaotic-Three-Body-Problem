"""
Evaluation script for Three-Body HNN.

SAME EVALUATION AS DNN/PINN/LNN - direct position prediction, same metrics.
Plus HNN-specific metrics (Hamiltonian vector-field residual, Hamiltonian MSE).
"""
...
from model import ThreeBodyHNN
from dataset import HNNTrajectoryDataset
from metrics import (
    ...
    compute_energy,
    # HNN-specific (Tier B)
    evaluate_trajectory_hnn,
    aggregate_metrics_hnn,
    print_metrics_hnn
)


def load_model(checkpoint_path, device='cuda'):
    """Load trained HNN model from checkpoint."""
...
def predict_trajectory(model, ic, t, norm_stats, device='cuda'):
    ...
    SAME AS DNN/LNN - direct query at each time step.
...
def evaluate_model(model, data_path, norm_stats, device='cuda', split='test',
                   include_hnn_metrics=True):
    """
    Evaluate model on all test trajectories.

    Includes both Tier A (common) and Tier B (HNN-specific) metrics.

    Returns:
        all_metrics: list of per-trajectory metrics
        aggregated: aggregated statistics
    """
    dataset = HNNTrajectoryDataset(data_path, split=split)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        if include_hnn_metrics:
            metrics = evaluate_trajectory_hnn(
    ...
    if include_hnn_metrics:
        aggregated = aggregate_metrics_hnn(all_metrics)
    else:
...
def plot_training_history(history_path, save_path=None):
    ...
    if 'hamilton_loss' in history:
        ax.semilogy(history['hamilton_loss'], label='Hamilton Eqs', alpha=0.8)
    if 'ham_sup_loss' in history:
        ax.semilogy(history['ham_sup_loss'], label='H Supervision', alpha=0.8)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
...
def plot_sample_predictions(model, data_path, norm_stats, device='cuda',
    ...
    dataset = HNNTrajectoryDataset(data_path, split='test')
    ...
    for row_idx, data_idx in enumerate(indices):
        ...
        ax_pred.set_title(f'Trajectory {sample["traj_idx"]} - HNN Prediction')
    ...
    plt.suptitle('HNN: Ground Truth vs Prediction (o=start)', fontsize=14, y=1.01)
    plt.tight_layout()
...
def save_results(aggregated, all_metrics, output_dir):
    ...
    # HNN-specific Tier B metrics
    if 'hamiltonian_residual' in all_metrics[0]:
        per_traj['hamiltonian_residual'] = [m['hamiltonian_residual'] for m in all_metrics]
    if 'hamiltonian_mse' in all_metrics[0]:
        per_traj['hamiltonian_mse'] = [m['hamiltonian_mse'] for m in all_metrics]

    np.savez(output_dir / 'per_trajectory_metrics.npz', **per_traj)


def main():
    parser = argparse.ArgumentParser(description='Evaluate Three-Body HNN')
    ...
    # Print results (Tier A + Tier B HNN-specific)
    print_metrics_hnn(aggregated, model_name="HNN")
