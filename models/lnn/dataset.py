"""
PyTorch Dataset for LNN training on Three-Body Problem.

SAME FORMAT AS DNN/PINN:
- Input: [t, x2_0, z2_0]
- Target: [x1_t, z1_t, x2_t, z2_t]

For LNN physics loss, we also provide collocation points like PINN.
"""
...
class LNNDataset(Dataset):
    """
    Dataset for LNN position prediction (SAME AS DNN).
...
class LNNCollocationDataset(Dataset):
    """
    Collocation points for LNN physics loss.

    Generates random (t, IC) pairs for computing E-L residual.
    """

    def __init__(self, train_dataset, n_collocation=10000, seed=42):
        """
        Args:
            train_dataset: LNNDataset (training set) for domain bounds
...
class LNNTrajectoryDataset(Dataset):
    """
    Dataset returning full trajectories for LNN evaluation.
    SAME AS DNN's ThreeBodyTrajectoryDataset.
    """
...
def create_lnn_dataloaders(data_path, batch_size=2048, n_collocation=10000, num_workers=0):
    """
    Create train/val/test dataloaders for LNN.

    Returns:
        train_loader, val_loader, test_loader, collocation_loader, norm_stats
    """
    train_ds = LNNDataset(data_path, split='train', normalize=True)
    val_ds = LNNDataset(data_path, split='val', normalize=True)
    test_ds = LNNDataset(data_path, split='test', normalize=True)
    ...
    # Collocation dataset
    collocation_ds = LNNCollocationDataset(train_ds, n_collocation=n_collocation)
...
if __name__ == "__main__":
    ...
    print("Testing LNNDataset (DNN-compatible)...")
    train_ds = LNNDataset(data_path, split='train')
    val_ds = LNNDataset(data_path, split='val')
    test_ds = LNNDataset(data_path, split='test')
    ...
    print("\n" + "="*50)
    print("Testing LNNCollocationDataset...")
    coll_ds = LNNCollocationDataset(train_ds, n_collocation=1000)
    ...
    print("\n" + "="*50)
    print("Testing LNNTrajectoryDataset...")
    traj_ds = LNNTrajectoryDataset(data_path, split='test')
