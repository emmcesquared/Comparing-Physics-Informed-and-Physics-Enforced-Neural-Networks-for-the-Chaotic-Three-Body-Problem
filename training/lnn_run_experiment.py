"""
LNN Experiment Launcher with Timestamped Folders.
...
This allows systematic exploration of LNN architectures for the three-body problem.
        ...
        if folder.is_dir():
            ...
            if metrics_file.exists():
                with open(metrics_file) as f:
                    ...
                    info["el_residual_mean"] = metrics.get("el_residual_mean", "?")

            experiments.append(info)

    # Print table
    print("\n" + "="*100)
    print("LNN EXPERIMENTS")
    print("="*100)
    print(f"{'Name':<30} {'Note':<20} {'Hidden':<8} {'Layers':<8} {'MAE':<12} {'EL Res':<12}")
    print("-"*100)

    for exp in experiments:
        mae = f"{exp.get('mae_mean', '?'):.4f}" if isinstance(exp.get('mae_mean'), float) else "?"
        el_res = f"{exp.get('el_residual_mean', '?'):.4f}" if isinstance(exp.get('el_residual_mean'), float) else "?"
        print(f"{exp['name']:<30} {exp.get('note', ''):<20} {exp.get('hidden_dim', '?'):<8} {exp.get('num_layers', '?'):<8} {mae:<12} {el_res:<12}")
...
def main():
    parser = argparse.ArgumentParser(description='LNN Experiment Launcher')
    ...
    parser.add_argument('--lagrangian-hidden', type=int, default=64,
                        help='Hidden dimension for Lagrangian network')
    parser.add_argument('--lagrangian-layers', type=int, default=3,
                        help='Number of layers for Lagrangian network')
    ...
    parser.add_argument('--lambda-el', type=float, default=0.01,
                        help='Weight for Euler-Lagrange loss')
    parser.add_argument('--lambda-lag', type=float, default=1.0,
                        help='Weight for Lagrangian supervision loss')
